...
export async function GET() {
  ...
  const configured = localAiConfigured(visionModel());
  return json({
    configured,
    signedIn: !!user,
...
export async function POST(req: Request) {
  ...
  if (!user)
    return json(
      {
        error:
          "Өз серверіміздегі AI арқылы тану үшін аккаунтпен кір. Төмендегі қолмен таңдау жұмыс істейді.",
      },
      401,
    );
  const model = visionModel();
  if (!localAiConfigured(model))
    return json(
      {
        error:
          "Өз серверіміздегі AI тануы әлі бапталмаған. Сурет сақталмады — сөзді қолмен таңда.",
      },
      503,
    );
  ...
  try {
    ...
    return json(
      {
        error: busy
          ? "Досжанға қазір сұрау көп. Біраздан кейін қайта байқап көр."
          : "Досжан затты тани алмады. Қайта байқап көр немесе сөзді қолмен таңда.",
      },
      busy ? 429 : 503,
    );
  } finally {
